...
async function POST(req) {
    ...
    try {
        ...
        const humanMessage = new messages_1.HumanMessage(lastMessage.content);
        ...
        const stream = new ReadableStream({
            async start(controller) {
                try {
                    ...
                    const streamResult = await graph_1.app.stream({ messages: [humanMessage] }, { streamMode: "values" });
                    ...
                    for await (const event of streamResult) {
                        ...
                        const toolCalls = lastMsg instanceof messages_1.AIMessage ? lastMsg.tool_calls : undefined;
                        // Extract text response (only if no tool calls - means it's the final answer)
                        if (lastMsg instanceof messages_1.AIMessage && !toolCalls && lastMsg.content) {
