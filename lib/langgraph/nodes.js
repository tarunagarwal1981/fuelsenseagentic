...
exports.reducerNode = reducerNode;
...
// Bind tools to LLM
const llmWithTools = llm.bindTools(tools_1.tools);
...
async function reducerNode(state) {
    ...
    const toolMessages = state.messages.filter((msg) => msg instanceof messages_1.ToolMessage);
        ...
        try {
            ...
            if (toolName === 'calculate_route' && toolResult) {
                ...
                if (portsArray.length > 0) {
                    updates.ports = portsArray.map((p) => ({
                ...
                if (pricesByPort && typeof pricesByPort === 'object') {
                    updates.prices = Object.entries(pricesByPort).map(([portCode, priceData]) => {
                        const prices = Array.isArray(priceData) ? priceData : [priceData];
                        return {
                            port_code: portCode,
                            port_name: prices[0]?.price?.port_name || prices[0]?.port_name || portCode,
                            prices: {
                                VLSFO: prices.find((p) => p.price?.fuel_type === 'VLSFO' || p.fuel_type === 'VLSFO')?.price?.price_per_mt || prices.find((p) => p.fuel_type === 'VLSFO')?.price_per_mt,
                                LSGO: prices.find((p) => p.price?.fuel_type === 'LSGO' || p.fuel_type === 'LSGO')?.price?.price_per_mt || prices.find((p) => p.fuel_type === 'LSGO')?.price_per_mt,
                                MGO: prices.find((p) => p.price?.fuel_type === 'MGO' || p.fuel_type === 'MGO')?.price?.price_per_mt || prices.find((p) => p.fuel_type === 'MGO')?.price_per_mt,
                            },
