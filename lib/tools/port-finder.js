...
exports.portFinderToolSchema = exports.PortFinderError = exports.portFinderInputSchema = void 0;
exports.haversineDistance = haversineDistance;
exports.findPortsNearRoute = findPortsNearRoute;
exports.executePortFinderTool = executePortFinderTool;
const zod_1 = require("zod");
/**
 * Zod schema for input validation
 */
exports.portFinderInputSchema = zod_1.z.object({
    route_waypoints: zod_1.z
...
async function loadPortsData() {
    ...
    try {
        ...
        portsCache = ports.filter((p) => p.fuel_capabilities &&
            Array.isArray(p.fuel_capabilities) &&
            p.fuel_capabilities.length > 0);
        return portsCache;
    }
    catch (error) {
        throw new PortFinderError(`Failed to load ports data: ${error instanceof Error ? error.message : 'Unknown error'}`, 'PORT_DATA_LOAD_ERROR');
    }
}
/**
 ...
 */
function findNearestWaypoint(port, waypoints) {
 ...
 */
async function findPortsNearRoute(input) {
    // Validate input using Zod schema
    const validatedInput = exports.portFinderInputSchema.parse(input);
    ...
    try {
        ...
        const foundPorts = Array.from(portMap.values()).sort((a, b) => a.distance_from_route_nm - b.distance_from_route_nm);
        ...
        if (error instanceof zod_1.z.ZodError) {
            throw new PortFinderError(`Input validation failed: ${error.issues.map((e) => e.message).join(', ')}`, 'VALIDATION_ERROR');
        }
        // Handle unexpected errors
        throw new PortFinderError(`Unexpected error during port finding: ${error instanceof Error ? error.message : 'Unknown error'}`, 'UNEXPECTED_ERROR');
    }
}
/**
 ...
 */
async function executePortFinderTool(args) {
    return findPortsNearRoute(args);
}
