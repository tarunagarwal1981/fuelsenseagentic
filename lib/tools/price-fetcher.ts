 ...
 * Uses the service layer for price data access with 3-tier caching (Cache → DB → JSON fallback).
 * 
 * Features:
 ...
 * - Currency formatting
 * - 3-tier caching via PriceRepository
 */
 ...
 * 3. Fetches latest prices for each port using PriceRepository (with 3-tier caching)
 * 4. Checks price freshness and generates warnings
 * 5. Formats prices with currency
 * 6. Returns organized price data
 * 
 * Refactored to use PriceRepository with 3-tier caching:
...
export async function fetchPrices(
  ...
  try {
    ...
    for (const portCode of port_codes) {
      try {
        // Get latest prices for this port (handles caching automatically)
