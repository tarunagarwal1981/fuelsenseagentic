...
async function resolveEntitiesToCodes(
  ...
  if (entities.origin && !skipOriginDestination) {
    try {
      ...
      if (port) {
        ...
        console.log(`✅ [SUPERVISOR-RESOLVER] Origin resolved: ${entities.origin} → ${port.code} (${port.name})`);
        console.log(`   Coordinates: [${port.coordinates[0]}, ${port.coordinates[1]}]`);
      } else {
        console.warn(`⚠️  [SUPERVISOR-RESOLVER] Could not resolve origin: "${entities.origin}"`);
        console.warn(`   Will attempt regex-based extraction as fallback`);
      }
    } catch (error) {
      console.error(`❌ [SUPERVISOR-RESOLVER] Error resolving origin "${entities.origin}":`, error);
    }
  }
  ...
  if (entities.destination && !skipOriginDestination) {
    try {
      ...
      if (port) {
        ...
        console.log(`✅ [SUPERVISOR-RESOLVER] Destination resolved: ${entities.destination} → ${port.code} (${port.name})`);
        console.log(`   Coordinates: [${port.coordinates[0]}, ${port.coordinates[1]}]`);
      } else {
        console.warn(`⚠️  [SUPERVISOR-RESOLVER] Could not resolve destination: "${entities.destination}"`);
        console.warn(`   Will attempt regex-based extraction as fallback`);
      }
    } catch (error) {
      console.error(`❌ [SUPERVISOR-RESOLVER] Error resolving destination "${entities.destination}":`, error);
    }
  }
  ...
  if (entities.bunker_ports && entities.bunker_ports.length > 0) {
    ...
    for (const bunkerPortName of entities.bunker_ports) {
      try {
        console.log(`🔍 [SUPERVISOR-RESOLVER] Resolving bunker port: "${bunkerPortName}"`);
        const port = await portRepo.findByName(bunkerPortName);
        
        if (port) {
          resolved.bunker_ports.push(port.code);
          console.log(`✅ [SUPERVISOR-RESOLVER] Bunker port resolved: ${bunkerPortName} → ${port.code}`);
        } else {
          console.warn(`⚠️  [SUPERVISOR-RESOLVER] Could not resolve bunker port: "${bunkerPortName}"`);
        }
      } catch (error) {
        console.error(`❌ [SUPERVISOR-RESOLVER] Error resolving bunker port "${bunkerPortName}":`, error);
      }
    }
  }
  ...
  console.log(`📊 [SUPERVISOR-RESOLVER] Resolution complete: ${resolvedCount} ports resolved`);
...
export async function generateExecutionPlan(
  ...
  if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
    console.log('📋 [SUPERVISOR-PLANNER] Using cached execution plan');
    return cached.plan;
  }
...
- Vessel timeline: ${stateAnalysis.has_vessel_timeline ? '✅ Available' : '❌ Missing'}
- Weather forecast: ${stateAnalysis.has_weather_forecast ? '✅ Available' : '❌ Missing'}
- Weather consumption: ${stateAnalysis.has_weather_consumption ? '✅ Available' : '❌ Missing'}
  ...
  try {
    ...
    if (plan.extracted_entities) {
      ...
      try {
        ...
        console.error('❌ [SUPERVISOR-PLANNER] Entity resolution failed:', resolutionError);
        // Continue without resolved codes - agents will use fallback extraction
        plan.resolved_codes = {};
      }
    } else {
      console.warn('⚠️  [SUPERVISOR-PLANNER] No entities extracted from query - agents will use fallback extraction');
    }
    ...
    console.log('✅ [SUPERVISOR-PLANNER] Generated execution plan:', {
    ...
    console.error('❌ [SUPERVISOR-PLANNER] Plan generation failed:', error);
    throw error;
  }
}
...
function validatePlan(
  ...
  if (plan.resolved_codes && (plan.resolved_codes.origin || plan.resolved_codes.destination)) {
    console.log('🎯 [SUPERVISOR-PLANNER] Resolved port codes:', {
